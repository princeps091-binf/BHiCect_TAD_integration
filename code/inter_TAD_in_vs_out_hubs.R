...
inter_TAD_hic_dat_l<-map(1:nrow(TAD_pair_combo),function(i){
...
hub_subset_tbl<-do.call(bind_rows,map(1:nrow(hub_TAD_content),function(i){
  ...
  TAD_pair_combo %>% 
    filter(V1 %in% tmp_TAD_set & V2 %in% tmp_TAD_set | V2 %in% tmp_TAD_set & V1 %in% tmp_TAD_set) %>% 
...
TAD_pair_combo %>% 
  ...
  unnest(cols=c(hic.dat)) %>% 
  distinct %>% 
  left_join(.,hub_subset_tbl) %>% 
  mutate(hub.io=ifelse(is.na(hub.io),"out",hub.io)) %>% 
